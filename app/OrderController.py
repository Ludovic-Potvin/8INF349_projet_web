...
class OrderController() :
    ...
    def update(self, id, data, Session):
        ...
        if 'order' in data:
            return self.update_order_shipping(id, data, Session)
        elif 'credit_card' in data:
            return self.update_order_card(id, data, Session)
        else:
            return {"error": "tea - how did you end up here"}, 418

    # Description: Only update the shipping info and the email
    @classmethod
    def update_order_shipping(self, id, data, session):
        db_session = session()
    ...
    @classmethod
    def update_order_card(self, id, data, session):
        db_session = session()
